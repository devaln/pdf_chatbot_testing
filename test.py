...
def split_query(query):
    import re
    questions = re.split(r'\?\s+and\s+|\?\s*&\s*', query.strip('?'))
        ...
        with st.chat_message("assistant"):
            with st.spinner("Searching documents..."):
                ...
                for sq in sub_questions:
                    results = st.session_state.vs.similarity_search_with_score(sq, k=3)
                    doc_chunks = [doc.page_content for doc, _ in results]
                    all_context.extend(doc_chunks)

                context = "\n\n".join(all_context)
                prompt = f"""You are a helpful assistant. Answer all of the following questions clearly and accurately:

{chr(10).join([f"Q{i+1}: {q}" for i,q in enumerate(sub_questions)])}

Use only the context below:

{context}

Provide a numbered answer for each question.
"""
                llm = ChatOllama(model=OLLAMA_LLM_MODEL, base_url=OLLAMA_BASE_URL)
                response = llm.invoke(prompt)
                answer = response.content if hasattr(response, "content") else str(response)
